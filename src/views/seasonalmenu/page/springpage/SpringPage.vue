<template>
  <div>
    <div id="upload">
      <router-link to="/uploadspring" tag="button">发布春季菜谱</router-link>
    </div>
  <div class="box1">
    <div class="a-user right-user" v-for="value in currentPageData">
      <router-link :to="{path:'/springmenucontent',query:{id:value.menuId}}">
        <div class="picture">
      ...
      <div class="user-infro">
        <dl class="name-menu">
          <dt><span>NO.{{value.menuId}}</span><router-link :to="{path:'/springmenucontent',query:{id:value.menuId}}">{{value.menuName}}</router-link></dt>
        </dl>
        <button @click="collectBtn(value.menuId)" v-if="$store.state.springCollectId.indexOf(parseInt(value.menuId))===-1">收藏</button>
        <button @click="cancelCollectBtn(value.menuId)" v-else>取消收藏</button>
        <router-link tag="button" :to="{path:'/springcomment',query:{id:value.menuId}}">评论</router-link>
      </div>
    </div>
  </div>
  <p class="pages">
      ...
      if(localStorage.getItem("token")){
        request({
          ...
          for(let item of this.collectMenuList){
            this.$store.commit('addSpringCollectId',item.collectId);
          }
        }).catch(err => {
          console.log(err);
        });
