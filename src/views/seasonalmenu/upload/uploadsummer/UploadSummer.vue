<template>
  <div class="box">
    ...
    <button @click="submitForm($event)">提交</button>
      <router-link tag="button" to="seasonalmenu/summerpage" class="back">返回</router-link>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      submitForm(event) {
        ...
        request({
          ...
          console.log(res);
          alert("发布成功！");
          this.$router.push({path:'/summergmenucontent',query:{id:res.data.message.insertId}});
        }).catch(err => {
          alert("您已经收藏该菜谱了！");
          console.log(err);
        });
