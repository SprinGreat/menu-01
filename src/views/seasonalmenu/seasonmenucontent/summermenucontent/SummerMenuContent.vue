<template>
  <div>
    ...
    <p class="detailBottom">
      <button @click="collectBtn(content[0].menuId)" v-if="$store.state.summerCollectId.indexOf(parseInt(content[0].menuId))===-1">收藏</button>
      <button @click="cancelCollectBtn(content[0].menuId)" v-else>取消收藏</button>
      <router-link tag="button" :to="{path:'/summercomment',query:{id:content[0].menuId}}">评论</router-link>
      <button v-if="isUser" @click="deleteMenu">删除菜谱</button>
      <router-link v-if="isUser" tag="button" :to="{path:'/modifysummer',query:{id:content[0].menuId}}">修改菜谱</router-link>
      <router-link tag="button" to="/seasonalmenu/summerpage">返回</router-link>
    </p>
  </div>
</template>
    ...
    methods:{
      collectBtn(menuId) {
        if(localStorage.getItem('token')){
          this.$store.commit('addSummerCollectId', menuId);
          const obj = {
            menuId: menuId
          };
      ...
      cancelCollectBtn(menuId) {
        ...
        this.$store.state.summerCollectId.remove(parseInt(menuId));
        const obj = {
          menuId: menuId
        };
        const jon = JSON.stringify(obj);
        request({
          url: '/cancelSummerCollect',
          method: 'post',
          data: jon,
          headers: {'Content-Type': 'application/json;charset=UTF-8'}
        }).then(res => {
      ...
      deleteMenu(){
        ...
        request({
          ...
          console.log(res);
          alert("该菜谱已经删除成功了");
          this.$router.push('/seasonalmenu/summerpage');
        }).catch(err=>{
          console.log(err);
        });
      }
