<template>
  <div>
    ...
    <p class="detailBottom">
      <button @click="collectBtn(content[0].menuId)" v-if="$store.state.winterCollectId.indexOf(parseInt(content[0].menuId))===-1">收藏</button>
      <button @click="cancelCollectBtn(content[0].menuId)" v-else>取消收藏</button>
      <router-link tag="button" :to="{path:'/wintercomment',query:{id:content[0].menuId}}">评论</router-link>
      <button v-if="isUser" @click="deleteMenu">删除菜谱</button>
      <router-link v-if="isUser" tag="button" :to="{path:'/modifywinter',query:{id:content[0].menuId}}">修改菜谱</router-link>
      <router-link tag="button" to="seasonalmenu/winterpage">返回</router-link>
    </p>
  </div>
</template>
    ...
    methods:{
      ...
      deleteMenu(){
        ...
        request({
          ...
          console.log(res);
          alert("该菜谱已经删除成功了");
          this.$router.push('/seasonalmenu/winterpage');
        }).catch(err=>{
          console.log(err);
        });
      }
