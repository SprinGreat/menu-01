<template>
  ...
  <!--<input name="file" type="file" accept="image/png,image/gif,image/jpeg" @change="update"/><br>-->
    ...
    <button @click="submitForm($event)">提交</button>
      <router-link tag="button" to="/menu" class="back">返回</router-link>
    </div>
  </div>
</template>
<script>
    ...
    methods: {
      ...
      submitForm(event) {
        ...
        request({
          ...
          console.log(res);
          console.log(typeof res.data.message.insertId);
          alert("发布成功！");
          this.$router.push({path:'/menucontent',query:{id:res.data.message.insertId}});
        }).catch(err => {
          alert("发布失败！");
          console.log(err);
        });
